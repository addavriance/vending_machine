...
void Keypad::click(int click_x, int click_y) {
    ...
    if (char_pos < char_map.size() - 3) { // type
        if (display_text.starts_with("OK") || display_text.starts_with("ERR")) display_text = "";
        if (display_text.size() < 3) display_text+=char_map[char_pos];
    }
